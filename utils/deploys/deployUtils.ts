...
export async function updateFeeRecipient(
  ...
  const extensionAddress = await getContractAddress(feeSplitExtensionName);
  const extensionInstance = await instanceGetter.getFeeSplitExtension(extensionAddress);

  const managerAddress = await extensionInstance.manager();
  const managerInstance = await instanceGetter.getBaseManager(managerAddress);
  const currentOperator = await managerInstance.operator();

  const updateFeeRecipientData = extensionInstance.interface.encodeFunctionData("updateFeeRecipient", [newFeeRecipient]);
